...
response = 'response-edit-config-full-interfaces.xml'

def demo(host, user):
    with manager.connect(host=host, port=22, username='cisco', password='cisco', hostkey_verify=False) as m:
                    c = m.edit_config(open('config-10.101.125.1-interfaces.xml', 'r').read(), format='xml', target='candidate', default_operation='merge')
